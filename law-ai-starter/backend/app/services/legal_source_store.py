...
def get_legal_source_store_snapshot(*, prefer_database: bool = True) -> LegalSourceStoreSnapshot:
    ...
    if not prefer_database or not database_ready:
        fallback_detail = (
            "Admin is currently reading from the in-memory prototype catalog. " + detail
            if detail
            else "Admin is currently reading from the in-memory prototype catalog."
        )
    ...
    if session_factory is None:
        return _in_memory_snapshot(
            detail="Database is configured, but no session factory is available yet, so admin is using the in-memory prototype catalog.",
            persisted_record_count=persisted_record_count,
    ...
    try:
        ...
        if rows:
            ...
            return LegalSourceStoreSnapshot(
                ...
                persisted_record_count=persisted_record_count,
                detail=(
                    "Admin is reading from persisted database records. " + detail
                    if detail
                    else "Admin is reading from persisted database records."
                ),
                records=records,
            )
        ...
        return _in_memory_snapshot(
            detail=(
                "Database is ready, but the persisted catalog is empty, so admin is falling back to the in-memory prototype catalog. " + detail
                if detail
                else "Database is ready, but the persisted catalog is empty, so admin is falling back to the in-memory prototype catalog."
            ),
            persisted_record_count=persisted_record_count,
            database_ready=True,
            foundation_stage=foundation_stage,
        ...
        return _in_memory_snapshot(
            detail=f"Database-backed source reading failed ({exc}), so admin is using the in-memory prototype catalog.",
            persisted_record_count=persisted_record_count,
            database_ready=False,
